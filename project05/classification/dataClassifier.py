...
def enhancedFeatureExtractorDigit(datum):
    ...
    for i in range(len(pixels)):
        for j in range(1,len(pixels[i])):
            if pixels[i][j] != 0:
                nonzero += 1
                if firstLeft == None or j < firstLeft:
                    firstLeft = j
                if j <= (len(pixels) + 1) / 2:
                    aboveCenter += 1
    ...
    for n in range(10):
        if aspectRatio < 0.69:
            ...
            features[(n + 1) * 10] = False
    
    for n in range(1000, 1005):
        if percentRight < 0.27:
            features[n] = 1.0
        else:
            features[n] = 0.0
...
def enhancedPacmanFeatures(state, action):
    ...
    nearestGhostDistance = min(util.manhattanDistance(pos, PacmanPos) for pos in ghostPos)
    features['nearestghostdistance'] = nearestGhostDistance
    return features
